...
plyr::count(t1$support) # 2,993 yes votes out of 4477
plyr::count(t2$support) # 3918 yes votes out of 4491
succ <- c(2984, 3916)
totals <- c(4464, 4480)
...
plyr::count(t3$support) # 3495 yes votes out of 4465
succ <- c(3916, 3480)
totals <- c(4480, 4455)
...
bdata$ineq_rev <- -1.0245613054
bdata$ineq_rev[which(bdata$change_ineq == -1)] <- 1.0227813740
bdata$ineq_rev[which(bdata$change_ineq == 0)] <- -0.0008899657
...
d <- read.csv("long_bin_controls.csv", header = T)
...
model1b_c <- glmer(support ~ change_poorz + change_richz + change_ineqz + change_effz +  
...
# Democrats
model1c_c <- glmer(support ~ change_poorz + change_richz + change_ineqz + change_effz +  
...
# Republicans
model1d_c <- glmer(support ~ change_poorz + change_richz + change_ineqz + change_effz +  
...
model2b_c <- glmer(support~poorupz + poordabsz + richupz + richdabsz + 
                  scale(age) + scale(income) + scale(male) + scale(white) + rep_con +
...
model2c_c <- glmer(support~poorupz + poordabsz + richupz + richdabsz + 
                  scale(age) + scale(income) + scale(male) + scale(white) + repub +
                  (poorupz*richupz) + (poorupz*richdabsz) + (poordabsz*richupz) + (poordabsz*richdabsz) + 
...
model2d_c <- glmer(support~poorupz + poordabsz + richupz + richdabsz + 
                  scale(age) + scale(income) + scale(male) + scale(white) + dem +
                  (poorupz*richupz) + (poorupz*richdabsz) + (poordabsz*richupz) + (poordabsz*richdabsz) + 
