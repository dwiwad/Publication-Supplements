---
title: "NHB Study 1 - Cross National Multilevel Model"
author: "Dylan Wiwad"
...
# just get the counts
plyr::count(ineq_mis$country)
plyr::count(attrib_mis$country)
plyr::count(id_mis$country)
plyr::count(inc_mis$country)
plyr::count(relig_mis$country)
plyr::count(educ_mis$country)
```
...
key_cols <- c("ineq", "attrib", "ideology", "sex", "age", "educ", "inc.lad", "relig.import", "country")
...
```{r descrip, echo = TRUE}
plyr::count(wvs$attrib)
atts <- c(11740, 28291)
chisq.test(atts, y=NULL)

psych::describe(wvs$ineq)
psych::describe(wvs$inc.lad)
psych::describe(wvs$relig.import)

psych::describe(wvs$age)
plyr::count(wvs$sex)
```
...
summary(lm(zineq~zattrib, data=wvs))
